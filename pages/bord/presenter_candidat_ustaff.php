...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action'])) {
        ...
        if ($_POST['action'] === 'fetch_data') {
            ...
            $sql = "SELECT * FROM candidats WHERE type_candidat = 'Ustaff'";
            if ($category !== 'ALL') {
                $sql .= " AND categorie = ?";
            }
            if ($filterValue) {
        ...
        elseif ($_POST['action'] === 'fetch_categories') {
            ...
            $sql = "SELECT DISTINCT categorie FROM candidats WHERE type_candidat = 'Ustaff'";
        ...
        elseif ($_POST['action'] === 'place_order') {
            ...
            if ($stmt->execute()) {
                echo json_encode([
                    'status' => 'success',
                    'message' => 'Votre réservation a été enregistrée avec succès. Veuillez noter que la réservation des candidats ne garantit pas qu\'ils soient promis à votre entreprise. Elle sera confirmée que lorsque les contrats sont signés et procédures entièrement réglées conformément aux critères de notre plateforme Ustaff Canada.'
                ]);
            } else {
