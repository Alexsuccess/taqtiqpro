...
while ($row = $resulti->fetch_assoc()) {
    $statutCertifie = ($row['validation'] === '1') 
        ...
        : '<span class="text-red">Non Certifié</span>';
    ...
    $proceduress = ($row['procedure1'] === '1') 
        ? '<span class="text-green">En cours</span>' 
        : '<span class="text-red">Non en cours</span>';
...
$userCodes = htmlspecialchars($row['id'] ?? '');
    ...
    echo "<td>" . htmlspecialchars($row['nomn'] ?? '') . " " . htmlspecialchars($row['prenomn'] ?? '') . "</td>";
    echo "<td>" . htmlspecialchars($row['datee'] ?? '') . "</td>";
  
    echo '<td>' . $documentsStatus . '</td>';
    echo '<td>' . $validationStatus . '</td>';
    echo '<td>' . $paymentStatus . '</td>';
    echo '<td>' . $currentStep . '</td>';
    echo '<td>' . $proceduress . '</td>';
    echo '<td>' . $statutCertifie . '</td>';
    echo '<td>' . $documentsStatuss . '</td>';
    echo '<td>
    ...
    document.getElementById('sendEmailBtn').addEventListener('click', function() {
        ...
        var candidateList = Array.from(document.querySelectorAll('input[name="candidates[]"]:checked')).map(function(checkbox) {
            return {
                ...
                userCode: checkbox.getAttribute('data-userCode'),
...
var candidateDetails = candidateList.map(function(candidate) {
    return `
        ${candidate.prof} CAND2024
        Prénom: ${candidate.prenom}
        Langue parlée / écrite: ${candidate.parle} / ${candidate.ecrit}
        Dimplômé(s): Oui
        Permis de conduire: ${candidate.permi || ''}


        Lien du CV: 
        https://admin.izishope.com/pages/bord/voir_voir_plus2.php?code=${candidate.code}
