...
if (isset($_GET['code'])) {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $stmt = $conn->prepare("UPDATE candidats SET categorie = ?, nom = ?, prenom = ?, email = ?, sexe = ?, pays = ?, prof = ?, city = ?, specail = ?, exp = ?, permi = ?, enfant = ?, parle = ?, ecrit = ?   WHERE code = ?");
        $stmt->bind_param("sssssssssssssss", $categorie, $nom, $prenom, $email, $sexe, $pays, $prof, $city, $specail, $exp, $permi, $enfant, $parle, $ecrit,  $code);
        $stmt->execute();
        $stmt->close();
        ...
        // Mise à jour des parcours professionnels
        if (isset($_POST['parcours_professionnel2'])) {
            foreach ($_POST['parcours_professionnel2'] as $id => $parcours) {
                $intitule = $parcours['intitule'];
                $description = $parcours['description'];
                $outils = $parcours['outils'];
                $references2 = $parcours['references2'];

                if ($id == 'new') {
                    // Ajout d'un nouveau parcours professionnel
                    $stmt = $conn->prepare("INSERT INTO competencess2 (code, intitule, description, outils, references2) VALUES (?, ?, ?, ?, ?)");
                    $stmt->bind_param("sssss", $code, $intitule, $description, $outils, $references2);
                } else {
                    // Mise à jour d'un parcours professionnel existant
                    $stmt = $conn->prepare("UPDATE competencess2 SET intitule = ?, description = ?, outils = ?, references2 = ? WHERE id = ?");
                    $stmt->bind_param("ssssi", $intitule, $description, $outils, $references2, $id);
                }
                $stmt->execute();
                $stmt->close();
            }
    ...
    // Récupération des parcours professionnels
    $stmt_professionnel2 = $conn->prepare("SELECT * FROM competencess2 WHERE code = ?");
    $stmt_professionnel2->bind_param("s", $code);
    $stmt_professionnel2->execute();
    $parcours_professionnel_result2 = $stmt_professionnel2->get_result();
...
<div class="form-group">
    ...
    <input type="text" class="form-control" id="parle" name="parle" value="<?php echo htmlspecialchars($candidat['parle'] ?? ''); ?>">
</div>
<div class="form-group">
    <label for="ecrit">Langue écrite:</label>
    <input type="text" class="form-control" id="ecrit" name="ecrit" value="<?php echo htmlspecialchars($candidat['ecrit'] ?? ''); ?>">
</div>
        ...
        <h2>Compétences</h2>
        <?php if ($parcours_professionnel_result2->num_rows > 0) { ?>
            <?php while ($parcours_professionnel2 = $parcours_professionnel_result2->fetch_assoc()) { ?>
                <div class="form-group">
                    <label for="poste_<?php echo $parcours_professionnel2['id']; ?>">Intitulé:</label>
                    <input type="text" class="form-control" id="poste_<?php echo $parcours_professionnel2['id']; ?>" name="parcours_professionnel2[<?php echo $parcours_professionnel2['id']; ?>][intitule]" value="<?php echo htmlspecialchars($parcours_professionnel2['intitule']); ?>">
                </div>
                <div class="form-group">
                    <label for="entreprise_<?php echo $parcours_professionnel2['id']; ?>">Déscription:</label>
                    <input type="text" class="form-control" id="entreprise_<?php echo $parcours_professionnel2['id']; ?>" name="parcours_professionnel2[<?php echo $parcours_professionnel2['id']; ?>][description]" value="<?php echo htmlspecialchars($parcours_professionnel2['description']); ?>">
                </div>
                <div class="form-group">
                    <label for="periode_<?php echo $parcours_professionnel2['id']; ?>">Outils:</label>
                    <input type="text" class="form-control" id="periode_<?php echo $parcours_professionnel2['id']; ?>" name="parcours_professionnel2[<?php echo $parcours_professionnel2['id']; ?>][outils]" value="<?php echo htmlspecialchars($parcours_professionnel2['outils']); ?>">
                </div>
                <div class="form-group">
                    <label for="pays_prof_<?php echo $parcours_professionnel2['id']; ?>">Références:</label>
                    <input type="text" class="form-control" id="pays_prof_<?php echo $parcours_professionnel2['id']; ?>" name="parcours_professionnel2[<?php echo $parcours_professionnel2['id']; ?>][references2]" value="<?php echo htmlspecialchars($parcours_professionnel2['references2']); ?>">
                </div>
            <?php } ?>
        <?php } else { ?>
        ...
        <?php } ?>
        <div class="form-group">
            <button type="button" class="btn btn-secondary" onclick="addProfessionnel2()">Ajouter une Compétence</button>
        </div>
...
function addProfessionnel2() {
    const formGroup = document.createElement('div');
    formGroup.className = 'form-group';
    formGroup.innerHTML = `
        <label for="intitule_new">Intitulé:</label>
        <input type="text" class="form-control" id="intitule_new" name="parcours_professionnel2[new][intitule]">
        <label for="description_new">Déscription:</label>
        <input type="text" class="form-control" id="description_new" name="parcours_professionnel2[new][description]">
        <label for="outils_new">Outils:</label>
        <input type="text" class="form-control" id="outils_new" name="parcours_professionnel2[new][outils]">
        <label for="references2_new">Références:</label>
        <input type="text" class="form-control" id="references2_new" name="parcours_professionnel2[new][references2]">
    `;
    document.querySelector('form').insertBefore(formGroup, document.querySelector('form').querySelector('.form-group:last-child'));
}
</script>
