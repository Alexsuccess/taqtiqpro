...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("INSERT INTO candidats (type_candidat, categorie, nom, prenom, email, tel, prof, sexe, pays, city, exp, region, specail, ecrit, parle, permi, enfant, user_id, code) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, '1', ?)");
    $stmt->bind_param("ssssssssssssssssss", $type_candidat, $categorie, $nom, $prenom, $email, $tel, $prof, $sexe, $pays, $city, $exp, $region, $specail, $ecrit, $parle, $permi, $enfant, $randomCode);
...
$stmt_doc = $conn->prepare("INSERT INTO documentss (conseiller, nom, tel, courriel, diplome, cv, certificat_naissance, certificat_scolarite, passeport, attestation_etude, plan_cadre, attestation_enregistrement, releve_note, experience_professionnelle, permis_conduire, mandat_representation, acte_mariage, code) VALUES ('1', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
$stmt_doc->bind_param("sssssssssssssssss", $nom, $tel, $email, $fileNames['diplome'], $fileNames['cv'], $fileNames['certificat_naissance'], $fileNames['certificat_scolarite'], $fileNames['passeport'], $fileNames['attestation_etude'], $fileNames['plan_cadre'], $fileNames['attestation_enregistrement'], $fileNames['releve_note'], $fileNames['experience_professionnelle'], $fileNames['permis_conduire'], $fileNames['mandat_representation'], $fileNames['acte_mariage'], $randomCode);
...
<fieldset>
    ...
    <button type="button" class="btn btn-primary mt-3" id="addProfessional2">+</button>
        ...
        <fieldset>
            ...
            <input type="file" id="diplome" name="diplome" required="required"><br>
</div>
            <div class="form-group col-md-3">
            ...
            <input type="file" id="certificat_scolarite" name="certificat_scolarite" required="required"><br>
</div>
            <div class="form-group col-md-3">
