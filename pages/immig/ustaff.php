...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("INSERT INTO candidats (type_candidat, categorie, nom, prenom, email, tel, prof, typeposte, expiration, sexe, pays, city, exp, region, specail, ecrit, parle, permi, enfant, user_id, code) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, '1', ?)");
    $stmt->bind_param("ssssssssssssssssssss", $type_candidat, $categorie, $nom, $prenom, $email, $tel, $prof, $type_poste, $expiration, $sexe, $pays, $city, $exp, $region, $specail, $ecrit, $parle, $permi, $enfant, $randomCode);
    ...
    if ($stmt->execute()) {
        ...
        $stmt_user = $conn->prepare("INSERT INTO userss (compte, nom, prenom, prof, pays, Ville, email, telephone, password, profile_pic, type, statut) VALUES ('Candidat', ?, ?, ?, ?, ?, ?, ?, ?, 'papa.jpg', ?, '1')");
        $stmt_user->bind_param("sssssssss", $nom, $prenom, $prof, $pays, $city,  $email, $tel, $password, $type_candidat);
        $stmt_user->execute();

       $message = "Félicitations! Votre compte a été créé avec succès dans notre CRM. 
Veuillez vous connecter en utilisant votre adresse email et le mot de passe par défaut '1234'.
Pour vous connecter, utilisez le lien suivant : https://admin.izishope.com/login.php

Cordialement, 
L'équipe de UstaffCanada";
    } else {
...
<option value="français">Français</option>
    ...
    <option value="maldivien">Maldivien</option>
        ...
        <div class="form-group col-md-3">
            <label for="expiration">Date d'expiration du visa:</label>
            <input type="date" id="expiration" name="expiration" class="form-control">
        </div>

        <div class="form-group col-md-3">
            <label for="type_poste">Etes vous disponible pour tout type de poste de travail:</label>
    ...
    <div id="professionalInfo">
        <div class="form-row professional-entry">
            ...
            <div class="form-group col-md-3">
                ...
                <input type="text" class="form-control period" name="period[]">
            </div>
            <div class="form-group col-md-3">
                <label for="jobCountry">Pays:</label>
                <input type="text" class="form-control jobCountry" name="jobCountry[]">
            </div>
            <div class="form-group col-md-3">
