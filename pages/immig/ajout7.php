...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("INSERT INTO candidats (type_candidat, categorie, nom, prenom, email, tel, prof, sexe, pays, city, exp, region, specail, ecrit, parle, permi, enfant, hum, hum1, user_id, code, datee) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, '$userId', ?, now())");
    $stmt->bind_param("ssssssssssssssssssss", $type_candidat, $categorie, $nom, $prenom, $email, $tel, $prof, $sexe, $pays, $city, $exp, $region, $specail, $ecrit, $parle, $permi, $enfant, $hum, $hum1, $randomCode);

    if ($stmt->execute()) {
        $candidat_id = $stmt->insert_id; // Récupérer l'ID du candidat inséré

        // Insérer le parcours académique
        $stmt_academique = $conn->prepare("INSERT INTO parcours_academique (candidat_id, diplome, institution, date_obtention, user_id, code) VALUES (?, ?, ?, ?, $userId, ?)");
        $stmt_academique->bind_param("issss", $candidat_id, $diplome, $institution, $date_obtention, $randomCode);
        ...
        $stmt_professionnel = $conn->prepare("INSERT INTO parcours_professionnel (candidat_id, poste, entreprise, periode, pays, user_id, code) VALUES (?, ?, ?, ?, ?, $userId, ?)");
        $stmt_professionnel->bind_param("isssss", $candidat_id, $poste, $entreprise, $periode, $job_country, $randomCode);
        ...
        $stmt_competence = $conn->prepare("INSERT INTO competencess2 (candidat_id, intitule, description, outils, references2, user_id, code) VALUES (?, ?, ?, ?, ?, $userId, ?)");
        $stmt_competence->bind_param("isssss", $candidat_id, $intitule, $description, $outils, $references, $randomCode);
        ...
        // Créer un compte utilisateur lié au candidat
        $password = password_hash('1234', PASSWORD_BCRYPT);
        $stmt_user = $conn->prepare("INSERT INTO userss (compte, nom, prenom, prof, pays, Ville, email, telephone, password, profile_pic, type, statut, ext) VALUES ('Candidat', ?, ?, ?, ?, ?, ?, ?, ?, 'papa.jpg', ?, '1', '1')");
        $stmt_user->bind_param("sssssssss", $nom, $prenom, $prof, $pays, $city,  $email, $tel, $password, $type_candidat);
        $stmt_user->execute();
    ...
    echo "<script>
        document.addEventListener('DOMContentLoaded', function() {
            Swal.fire({
                title: 'Succès',
                text: 'Candidat(e) enregistré(e) avec succès!',
                icon: 'success',
...
$optionalFiles = ["cv", "attestation_etude", "plan_cadre", "attestation_enregistrement", "releve_note", "experience_professionnelle", "permis_conduire", "acte_mariage", "niu", "cassier"];
...
$stmt_doc = $conn->prepare("INSERT INTO documentss (conseiller, nom, tel, courriel, diplome, cv, certificat_naissance, certificat_scolarite, passeport, attestation_etude, plan_cadre, attestation_enregistrement, releve_note, experience_professionnelle, permis_conduire, mandat_representation, acte_mariage, niu, cassier, code) VALUES ('1', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
$stmt_doc->bind_param("sssssssssssssssssss", $nom, $tel, $email, $fileNames['diplome'], $fileNames['cv'], $fileNames['certificat_naissance'], $fileNames['certificat_scolarite'], $fileNames['passeport'], $fileNames['attestation_etude'], $fileNames['plan_cadre'], $fileNames['attestation_enregistrement'], $fileNames['releve_note'], $fileNames['experience_professionnelle'], $fileNames['permis_conduire'], $fileNames['mandat_representation'], $fileNames['acte_mariage'], $fileNames['niu'], $fileNames['cassier'], $randomCode);
...
<fieldset>
    ...
    <div class="row">
        <div class="form-group col-md-6">
            ...
            <select id="type_candidat" name="type_candidat" class="form-control">
                ...
                <option value="Ustaff">Résident sur le sol Canadien</option> 
            </select>
        </div>
        <div class="form-group col-md-6">
    ...
    <option value="Autre">Autre</option>
        ...
        <div class="form-group col-md-3">
            ...
            <input type="text" id="specail" name="specail" class="form-control" required="required">
        </div>
        <div class="form-group col-md-3">
    ...
    <option value="maldivien">Maldivien</option>
        ...
        <div class="form-group col-md-3">
            ...
            <select id="permi" name="permi" class="form-control">
                <option>Oui</option>
                <option>Non</option>
                <option>En processus</option>
            </select>
        </div>
        <div class="form-group col-md-3">
            <label for="hum">CCQ:</label>
            <select id="hum" name="hum" class="form-control">
                <option>Oui</option>
                <option>Non</option>
                <option>En processus</option>
            </select>
        ...
        <div class="form-group col-md-3">
            <label for="hum1">ASP:</label>
            <select id="hum1" name="hum1" class="form-control">
                <option>Oui</option>
                <option>Non</option>
                <option>En processus</option>
            </select>
        ...
        <div class="form-group col-md-3">
            ...
            <input type="number" id="enfant" name="enfant" class="form-control" required="required">
        </div>
    </div>
</fieldset>
    ...
    <div id="academicInfo">
        <div class="form-row academic-entry">
            <div class="form-group col-md-3">
                ...
                <input type="text" class="form-control degree" name="degree[]" required="required">
            </div>
            <div class="form-group col-md-3">
                <label for="institution">Institution:</label>
                <input type="text" class="form-control institution" name="institution[]" required="required">
            </div>
            <div class="form-group col-md-3">
                <label for="graduationDate">Date d'obtention:</label>
                <input type="text" class="form-control graduationDate" name="graduationDate[]" required="required">
            </div>
            <div class="form-group col-md-3">
    ...
    <div id="professionalInfo2">
        <div class="form-row professional2-entry">
            <div class="form-group col-md-3">
                ...
                <input type="text" class="form-control intitule" name="intitule[]" required="required">
            </div>
            <div class="form-group col-md-3">
                <label for="description">Description:</label>
                <input type="text" class="form-control description" name="description[]" required="required">
            </div>
            <div class="form-group col-md-3">
                <label for="outils">Outils / Technologies:</label>
                <input type="text" class="form-control outils" name="outils[]" required="required">
            </div>
            <div class="form-group col-md-3">
                <label for="references">Références :</label>
                <input type="text" class="form-control references" name="references[]" required="required">
            </div>
            <div class="form-group col-md-3">
...
<fieldset>
    ...
    <div class="form-row">
        ...
        <div class="form-group col-md-3">
            <label for="certificat_naissance">Acte de naissance:</label>
...
<script>
    document.getElementById('diplome_select').addEventListener('change', function () {
        ...
        <fieldset>
            ...
            <div class="form-group col-md-3">
            <label for="niu">Numéroe d'Identifiant Unique:</label>
            <input type="file" id="niu" name="niu"><br>
</div>
            <div class="form-group col-md-3">
            <label for="cassier">extrait de casier judiciaire:</label>
            <input type="file" id="cassier" name="cassier"><br>
</div>
</div>
